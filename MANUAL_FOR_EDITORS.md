...
🗂️ Управление файлами и очистка

Временные файлы и результаты

Система создает временные файлы в процессе рендеринга:

temp/ - Временные файлы (424MB в примере)
output/ - Готовые видео (151MB в примере)

Эти папки НЕ должны попадать в git репозиторий!

Очистка репозитория

Если репозиторий стал "тяжелым" из-за медиафайлов:
...
rm -rf temp/ output/
git rm -r --cached temp/ output/ _.mp4 _.mp3 _.jpg _.png

Что исключено из git (.gitignore):

Временные файлы: temp/, output/
Видео: _.mp4, _.mov, _.avi
Аудио: _.mp3, _.wav (кроме ассетов шаблонов)
Изображения: _.jpg, \*.png (кроме ассетов шаблонов)
After Effects auto-save файлы

Что ОСТАЕТСЯ в git:

Основные .aep файлы шаблонов
Файлы конфигурации (config.ts)
Ассеты шаблонов в папках Assets/
