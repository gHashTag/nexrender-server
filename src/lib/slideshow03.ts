...
const templatePath = join(templateDir, 'slideshow03', 'slideshow03.aet');
...
const imageAssets = imagePaths.map((path, index) => ({
  ...
  type: 'image' as const,
  composition: `Image ${String(index + 1).padStart(2, '0')}`,
  layerIndex: `${String(index + 1).padStart(2, '0')}.jpg`,
}));
...
const assets: readonly Asset[] = [
  {
    type: 'data',
    composition: 'SC 01 Extra Text',
    layerName: 'YOUR NEW SUBTITLE GOES HERE 01',
    property: 'Source Text',
    value: 'Condominium KATA',
  },
  {
    type: 'data',
    composition: 'SC 02 Extra Text',
    layerName: 'YOUR NEW SUBTITLE GOES HERE 02',
    property: 'Source Text',
